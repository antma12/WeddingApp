...
export class MassFunctionsComponent {
  ...
  constructor(
    private massFunctionService: MassFunctionService,
  ...
  submitForm = function (formValues) {
    ...
    let guestsX: [Observable<Guest>] = guestsInputValues.map(guest =>
      this.guestService.getGuest(guest)
    );
  ...
  forceSelection(element) {
    if (this.guestUsernames.indexOf(element.value) !== -1) {
      ...
      if (this.guestCollection === '') {
        ...
        this.guestService
          .getGuest(element.value)
          .subscribe(g => this.guestNames.push(g.firstName + ' ' + g.surname));
      }
  ...
  removeGuest(guestName: string) {
    ...
    this.guestCollection = this.guestCollection
      .split('###')
      .filter(
        el =>
          el !==
          (guestName.substr(0, 1) + guestName.split(' ')[1]).toLowerCase()
      )
      .join('###');
  }
}
