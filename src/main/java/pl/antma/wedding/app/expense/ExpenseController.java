...
public class ExpenseController {
    ...
    public ResponseEntity<Expense> findExpenseById(@PathVariable Long id){
        return expenseService.findExpenseById(id).map(ResponseEntity::ok).orElse(ResponseEntity.notFound().build());
    }

    @PutMapping("/{id}")
    public ResponseEntity<Expense> updateExpense(@RequestBody Expense expense, @PathVariable Long id) {
        return expenseService.updateExpense(expense, id).map(ResponseEntity::ok).orElse(ResponseEntity.notFound().build());
    }

    @DeleteMapping("/{id}")
    public ResponseEntity<Expense> deleteExpense(@PathVariable Long id) {
       return expenseService.deleteExpense(id).map(ResponseEntity::ok).orElse(ResponseEntity.notFound().build());
    }
}
