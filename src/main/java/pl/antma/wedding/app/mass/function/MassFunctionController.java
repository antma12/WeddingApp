...
public class MassFunctionController {
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<MassFunction> deleteMassFunction(@PathVariable Long id) {
        return massFunctionService.deleteMassFunction(id).map(ResponseEntity::ok).orElse(ResponseEntity.notFound().build());
    }
}
